function [min_cen_ru0] = ru0_dorsal_volar()
...
min_ru0_v = min_ru_v(:,2:2:length(min_ru_v)); % 18 total arms
min_ru0_d = min_ru_d(:,2:2:length(min_ru_d)); % 10 total arms

% transpose for later use

min_ru0_v = transpose(min_ru0_v);
min_ru0_d = transpose(min_ru0_d);
...
for row = 1:length(min_ru0_v);
        A = min_ru0_v(row,:); % store current arms minimum distance data for concatenation
        B = cendist_v(row,:); % store current arms centroid distance data for concatenation
        min_cen_ru0_v = [min_cen_ru0_v;A;B]; % concatenate
%         plot(min_ru0_v(row,:),cendist_v(row,:));
%         title('RU0 Minimum Distance vs Centroid Distance (volar)');
%         xlabel('Minimum Distance (m)');
%         ylabel('Centroid Distance (m)');
end

for row = 1:length(min_ru0_d);
        D = [min_ru0_d(row,:)]; % store current arms minimum distance data for concatenation
        E = [cendist_d(row,:)]; % store current arms centroid distance data for concatenation
        min_cen_ru0_d = [min_cen_ru0_d;D;E]; % concatenate
%         plot(min_ru0_d(row,:),cendist_d(row,:));
%         title('RU0 Minimum Distance vs Centroid Distance (dorsal)');
%         xlabel('Minimum Distance (m)');
%         ylabel('Centroid Distance (m)');
end

min_cen_ru0 = vertcat(min_cen_ru0_v, min_cen_ru0_d);
xlswrite(filename,min_cen_ru0,functionname)
toc
end